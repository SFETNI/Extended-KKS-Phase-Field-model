...
dtime_s  = 250    # = adimensional time step [-]
dtime_max=400           # = maximum dimensionless time step bound [s]
...
# %%
plt.plot(time_out[good_step:5:-1], array_energy_s[good_step:5:-1])
plt.xlabel('time')
...
# C-H
N=5 # pas
plt.plot(T_out[good_step+1:-1:N],np.abs(array_residus_KKS[good_step+1:-1:N]) , label=' Total', color='blue')
plt.legend(loc='center left', bbox_to_anchor=(1.05, 0.8))
plt.xlabel('Temperature (K)')
...
np.savez_compressed(os.path.join(Save_path, outfile),time_out=time_out,T_out=T_out,array_dtime_s=array_dtime_s,\
array_residus_KKS=array_residus_KKS,array_residus_KKS_energ_deriv=array_residus_KKS_energ_deriv,\
array_residus_KKS_CH=array_residus_KKS_CH,array_residus_KKS_AC=array_residus_KKS_AC,array_residus_KKS_temp_term=array_residus_KKS_temp_term,array_energy_s=array_energy_s)